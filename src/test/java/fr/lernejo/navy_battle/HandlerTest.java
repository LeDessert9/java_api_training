...
class HandlerTest {
    ...
    void postReqTest() throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:1234/api/game/start"))
            .POST(HttpRequest.BodyPublishers.ofString("{\"id\":\"1\", \"url\":\"http://localhost:" + 1234 + "\", \"message\":\"message test\"}")).build();
    ...
    void another_ping_cheking() throws IOException, InterruptedException {
        new Serveur(9867);
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:9867/api/game/start")).DELETE().build();
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
